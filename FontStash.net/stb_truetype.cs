...
namespace FontStashDotNet
{
	public static class Stb_truetype
	{
		...
		static int stbtt__find_table(byte[] data, int fontstart, string tag)
		{
			...
			var tabledir = (uint)(fontstart + 12);
			int i;
		...
		public static int stbtt_FindGlyphIndex(ref stbtt_fontinfo info, int unicode_codepoint)
		{
			...
			var format = ttUSHORT(data, (int)(index_map + 0));
			if (format == 0)
			{ // apple byte encoding
			...
			{ // standard mapping for windows fonts: binary search collection of ranges
				var segcount = (ushort)(ttUSHORT(data, (int)(index_map + 6)) >> 1);
				var searchRange = (ushort)(ttUSHORT(data, (int)(index_map + 8)) >> 1);
				var entrySelector = ttUSHORT(data, (int)(index_map + 10));
				var rangeShift = (ushort)(ttUSHORT(data, (int)(index_map + 12)) >> 1);
			...
			{
				var ngroups = ttULONG(data, (int)(index_map + 12));
				int low, high;
				low = 0;
				high = (int)ngroups;
				...
				while (low < high)
				{
					...
					var start_char = ttULONG(data, (int)(index_map + 16 + mid * 12));
					var end_char = ttULONG(data, (int)(index_map + 16 + mid * 12 + 4));
					...
					{
						var start_glyph = ttULONG(data, (int)(index_map + 16 + mid * 12 + 8));
						if (format == 12)
		                                      ...
		                                      out stbtt_vertex[] pvertices)
		{
			...
			var g = stbtt__GetGlyfOffset(ref info, glyph_index);
			...
			if (numberOfContours > 0)
			{
				...
				points = data;
				var iPoints = g + 10 + numberOfContours * 2 + 2 + ins;
		...
		public static void stbtt_MakeGlyphBitmapSubpixel(ref stbtt_fontinfo info, byte[] output, int iOutput, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int glyph)
		{
			...
			stbtt_vertex[] vertices = null;
			var num_verts = stbtt_GetGlyphShape(ref info, glyph, out vertices);
			stbtt__bitmap gbm;   
		                            ...
		                            stbtt_vertex[] vertices, int num_verts, float scale_x, float scale_y, float shift_x, float shift_y, int x_off, int y_off, int invert, object userdata)
		{
			...
			var windings = stbtt_FlattenCurves(vertices, num_verts, flatness_in_pixels / scale, out winding_lengths, ref winding_count, userdata);
			if (windings != null)
			{
		                             ...
		                             int invert, object userdata)
		{
			...
			stbtt__edge[] e;
			int n, i, j, k, m;
			var vsubsample = result.h < 8 ? 15 : 5;
		...
		{
			var activeIsNext = new stbtt__active_edge();
			;
			int y, j = 0, eIndex = 0;
			var max_weight = (255 / vsubsample);  // weight per vertical scanline
			...
			while (j < result.h)
			{
				...
				for (s = 0; s < vsubsample; ++s)
				{
					...
					while (e[eIndex].y0 <= scan_y)
					{
						...
						{
							var z = new_active(e[eIndex], off_x, scan_y, userdata);
				...
				for (var cont = 0; cont < result.w; cont++)
		...
		{
			for (var cont = 0; cont < firstCount; cont++)
